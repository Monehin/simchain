...
4. ChainManager: Lock SOL on Solana via SolanaChain
5. ChainManager: Send cross-chain message via Hyperbridge
6. ChainManager: Mint DOT on Polkadot via PolkadotChain
7. Response: Cross-chain transaction IDs
```
...
export class ChainManager {
  ...
  private hyperbridge: HyperbridgeAdapter;
  
  constructor() {
    this.chains = new Map();
    this.solanaChain = new SolanaChain(solanaConfig);
    this.hyperbridge = new HyperbridgeAdapter(hyperbridgeConfig);
  ...
  async crossChainTransfer(transfer: CrossChainTransfer): Promise<CrossChainResult> {
    ...
    const sourceTx = await sourceChain.lockFunds(transfer);
    
    // 3. Send cross-chain message via Hyperbridge
    const message = await this.hyperbridge.sendMessage(transfer);
    
    // 4. Execute on target chain
    const targetChain = this.chains.get(transfer.targetChain);
    const targetTx = await targetChain.releaseFunds(transfer, message);
...
### **Hyperbridge's Role**
Hyperbridge is a **cross-chain messaging service**, not a chain manager. It provides:

- **ISMP Protocol**: Inter-System Message Passing between blockchains
- **Secure Messaging**: Cryptographically verified cross-chain communication
- **Relay Network**: Permissionless relayers for message delivery
- **State Verification**: Proof of message delivery and execution

**What Hyperbridge DOES:**
- Enables messages to pass between Solana ↔ Polkadot
- Enables messages to pass between Solana ↔ Ethereum
- Enables messages to pass between Solana ↔ Polygon
- Provides secure, verifiable cross-chain communication

**What Hyperbridge DOES NOT:**
- Manage individual chains (that's ChainManager's job)
- Validate user PINs (that's Solana's job)
- Execute transactions (that's individual chain clients' job)
- Store user data (that's your database's job)
...
export class HyperbridgeAdapter {
  ...
  async sendMessage(transfer: CrossChainTransfer): Promise<ISMPMessage> {
    // Send ISMP message for cross-chain communication
    const message = await this.sendISMPMessage(transfer);
    return message;
  }
  
  async waitForConfirmation(messageId: string): Promise<boolean> {
    // Wait for message confirmation on target chain
    return await this.confirmMessage(messageId);
  }
  ...
  private async confirmMessage(messageId: string): Promise<boolean> {
    // Implementation depends on Hyperbridge SDK
  }
}
```

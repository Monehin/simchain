...
import { ethers } from 'ethers'; // Updated for v6
...
export class HyperbridgeService {
  ...
  private async queryEVMStorage(chain: EvmChain, storageKey: string): Promise<unknown> {
    ...
    try {
      ...
      if (ethers.isAddress(contract) && slot) {
        // Use public RPC for demo
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const provider = new ethers.JsonRpcProvider((chain as any).url || 'https://eth.llamarpc.com');
